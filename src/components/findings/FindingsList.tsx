...
export const FindingsList = ({ dataset, onOpenFinding }: { dataset: Dataset; onOpenFinding: (id: string) => void; onOpenConstruct: (id: string) => void }) => {
  ...
  return <div className="column analysis-page">
    <header className="report-heading"><p className="section-label">Course analysis · All findings</p></header>
    ...
    <p className="analysis-small" role="status">{visible.length} {visible.length === 1 ? 'finding' : 'findings'} shown. A finding’s status comes from the current evidence; a missing finding is not proof that everything is working.</p>
